...
export const getProductoo = async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT * FROM productos WHERE producto_descripcion LIKE '%${searchText}%' AND producto_estatus_baja = 0 LIMIT 15`);
    res.json(result);
  } catch (error) {
    console.error(error);
...
export const addServicio = async (req, res) => {
  ...
  try {
    ...
    const insumoInsert = await pool.query("INSERT INTO cotizaciones_insumos SET ?", [insumo]);
    return res.redirect('/dashboard/operacion/proyectos/editar/'+req.body.insumo_cotizacion_id)
  } catch (error) {
    console.log(error);
