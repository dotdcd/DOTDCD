...
export const renderEmployee = async(req, res) => {
    try {
        ...
        for(let i = 0; i < filesNeeded.length; i++) {
            ...
            files.push((file[0].length > 0) ? [file[0][0].type, file[0][0].file, '<a class="btn btn-outline-primary" href="/dashboard/documento/'+file[0][0].id+'"><i class="fal fa-file-search"></i></a><a class="btn btn-outline-danger" onclick="showModalDel('+file[0][0].id+')" ><i class="fal fa-trash-alt"></i></a>'] : [filesNeeded[i], '<span class="badge badge-warning badge-pill">Documento no disponible</span>', '<input type="file" name="'+filesNeeded[i]+'" />'])
        }
        let documents = []
        const docs = await pool.query("SELECT * FROM documentos WHERE userId = ?", [id])
        for (const doc of docs[0]) {
            documents.push([doc.file, '<a class="btn btn-outline-primary" href="/dashboard/documento/v/'+doc.id+'"><i class="fal fa-file-search"></i></a> <br> <a  class="btn btn-outline-danger" onclick="showModalDelV('+doc.id+')" ><i class="fal fa-trash-alt"></i></a>'])
        }
...
const getActualContracts = async () => {
    try {
        ...
        let contratos = []
        for (const c of contracts[0]) {
            contratos.push([c.nombre, c.apellidos, c.departamento, c.sucursal, c.empresa, c.periodo+' meses', c.sueldo, c.inicio, c.vencimiento, c.fecha_firma, c.reconocimiento, "<a href='/dashboard/contabilidad/empleados/ver/contrato/"+c.empleado_id+"'><button class='btn btn-outline-success'>Ver Contrato</button></a>"])
        }
        return contratos
    } catch (error) {
        console.log(error)
...
const getErogaciones = async () => {
    try {
        const erogaciones = await pool.query("SELECT erogacion_id as id, erogacion_descripcion as descripcion FROM erogaciones")

        return erogaciones[0]
    } catch (error) {
        console.log(error)
    }
}
...
const getObra = async () => {
    try {
        const obras = await pool.query("SELECT erogacion_id as id, erogacion_descripcion as descripcion FROM erogaciones_obra")
        return obras[0]
    } catch (error) {
        console.log(error)
    }
...
export const renderEnuevos = async (req, res) => {
    try {
        ...
        res.render('contabilidad/bancos/egresos/nuevo', {empresas, proveedores, cotizaciones, cuentas, centroCostos, sucursal, inversion, erogacion, obras})
    } catch (error) {
        console.log(error)
    }
}
...
export const renderEbuscar = async (req, res) => {
    try {
        const egresos = await getVEgresos()
        return res.render('contabilidad/bancos/egresos/buscar', {egresos})
    } catch (error) {
        console.log(error)
    }
}
...
const getClientesInv = async () => {
    try {
        const clientes = await pool.query("SELECT cliente_id as id, cliente_razon_social as cliente FROM clientes WHERE cliente_estatus_baja = 0")
        return clientes[0]
    } catch (error) {
        console.log(error)
    }
...
export const renderIngresosbuscar = async (req, res) => {
    try {
